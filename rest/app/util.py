...
def query(
    database: str, query_txt: str, args=tuple(), fetchall=False
) -> Union[List, Tuple]:
    ...
    if not result:
        raise NoDataException(f"{database}: '{query_txt}' {args}")
...
def get_services(date: date, num_services: int) -> List[Dict[str, str]]:
    ...
    return [dict(zip(["date", "commemoration", "description"], r)) for r in results]
